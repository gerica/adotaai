    ...
    if (value) {
        ...
        value.contactList.forEach((contact, index) => {
            ...
            if (!contact || !contact.email) {
                ...
                const duplicate = value.contactList.filter((e, i) =>
                    index !== i && contact.email && e.email && contact.email.toLowerCase() === e.email.toLowerCase()
                );
    ...
    if (value.measures && value.measures.length > 0) {
        ...
        value.measures.forEach((measure, index) => {
            ...
            if (!measure || !measure.measure) {
                ...
                const duplicate = value.measures.filter((e, i) =>
                    index !== i && measure.measure && e.measure && measure.measure === e.measure
                );
                if (duplicate && duplicate.length > 0) {
                ...
                if (value.measureMain === measure.measure) {
                    measureErrors.measure = 'Unidade de medida idêntica à unidade de medida principal. Favor selecionar outra.';
                    measuresArrayErrors[index] = measureErrors;
                }
            }
            if (!measure || !measure.measure) {
            ...
            if (!isEmpty(measure.measurementUnitFactor) && !isNumber(parseFloat(measure.measurementUnitFactor))) {
